...
def autotest(permutations_list, start, end):
    ...
    for i in range(start, end):
        ...
        coverage = WebDriverWait(driver, 100).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, ".coverage .grade"))).text
        bulk = driver.find_element(By.CSS_SELECTOR, ".bulk .grade").text
        safety = driver.find_element(By.CSS_SELECTOR, ".safety .grade").text
        consistency = driver.find_element(
            By.CSS_SELECTOR, ".consistency .grade").text
        threat_score = driver.find_element(
            By.CSS_SELECTOR, ".threat-score").text
        ...
        remove = WebDriverWait(driver, 100).until(EC.element_to_be_clickable(
            (By.XPATH, "/html/body/div[1]/div/div[3]/div[1]/div/a")))
        remove.click()

        confirm = WebDriverWait(driver, 100).until(EC.element_to_be_clickable(
...
if __name__ == "__main__":
    ...
    # cast back to tuples
    permutations_list = [tuple(pokemon) for pokemon in permutations_list[len(permutations_list)//11*EXEC_TIME:len(permutations_list)//11*(EXEC_TIME+1)]]
